...
# Seuils
bearish_slope_threshold = -2  # seuil pour détecter une pente baissière forte
    ...
    for i in range(151, len(initial_data)):  # Commencer après les 150 premières bougies
        ...
        if len(tema20_window) > 2:
            ...
            if len(slopes_window) > 2:
                ...
                if  last_buy_price < initial_data[i] * 1.00075 and current_position == "buy"  and tema20 > tema50 and tema20 <= (last_tema + 8) and last_data <=  (last_datas + 30) and btc_balance >= 0.0001: #last_buy_price < initial_data[i] * 1.00075
                    if slope is not None and slope < bearish_slope_threshold and slopes_window[-2] > 0:
                ...
                if current_position == "none" and tema20 < tema50 and tema20 >= (last_tema + 8) and last_data >= (last_datas + 10) and usdt_balance >= 10.00: # and initial_data[i] <= 96500.0:
