...
# Paramètres globaux
symbol = 'BTCUSDT'
interval = Client.KLINE_INTERVAL_1SECOND  # Intervalle de 15 minutes
...
def get_historical_data():
    global close_window, tema20_window, tema50_window, slopes20_window, slopes50_window
...
def handle_new_candle():
    ...
    if len(tema20_window) > 2 and len(tema50_window) > 2:
        ...
        next_candle_time = (current_time // 1 + 1) * 1  # Prochain multiple de 900s (15min)
        ...
        if len(slopes20_window) > 2 and len(slopes50_window) > 2:
            ...
            if current_position == "buy" and tema20 > tema50 and btc_balance >= 0.000001 and tema20_window[-2] > tema50_window[-2] and tema20_window[-3] > tema50_window[-3] and last_buy_price * 1.00075 < close_price: 
                if slope20 is not None and slope50 is not None and slope20 < 0 and slope50 > 0 and slopes20_window[-2] > 0 and slopes50_window[-2] > 0 and slopes20_window[-3] > 0 and slopes50_window[-3] > 0:
            ...
            if current_position == "none" and tema20 < tema50 and usdt_balance >= 1 and tema20_window[-2] < tema50_window[-2] and tema20_window[-3] < tema50_window[-3]:
                if slope20 is not None and slope50 is not None and slope20 > 0 and slope50 < 0 and slopes20_window[-2] < 0 and slopes50_window[-2] < 0 and slopes20_window[-3] < 0 and slopes50_window[-3] < 0 and last_buy_price * 1.00075 > close_price:
                    ...
                    print(f"🔥ACHAT à 💰{close_price:.6f}, 🪙: {btc_balance:.6f}, 💲 : {usdt_balance:.6f}, prix de vente {last_buy_price:.2f}")
